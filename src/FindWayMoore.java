...
public class FindWayMoore implements IFindWay{

    final static int[][] cellAroundMoore = {
    ...
    public void findWay(Field field) throws IOException {
        ...
        Cell cellNow = field.startCell.copy();
        int borderLeft = y - 1;
        int borderRight = y + 1;
        int borderUp = x - 1;
        int borderDown = x + 1;
        ...
        while (!isFinalCellFind) {
            cellNow.coordinateX = x;
            cellNow.coordinateY = y;
            count++;
            if (field.isValidCoordinates(x, y)) {
                cellNow.stepFromStart = field.getCell(x, y).stepFromStart;
                fillCellAround(field, cellNow);
                Printer.printFieldToFile(field.getField(), 1);
            }
